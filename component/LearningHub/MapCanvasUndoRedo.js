/*v3.3: Color Selection + Undo Button
Choose from a set of predefined colors for drawing and undo last path.
*/
...
const MapCanvas = forwardRef(({ currentImageIndex, isAnnotationMode }, ref) => {
  ...
  // Undo function: removes last drawn path
  const undoLastPath = () => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Controls Bar: Zoom + Undo */}
      <ControlsBar zoomIn={zoomIn} zoomOut={zoomOut} undo={undoLastPath} />
    </SafeAreaView>
  );
});

// ControlsBar component with Zoom and Undo buttons
const ControlsBar = ({ zoomIn, zoomOut, undo }) => {
  return (
